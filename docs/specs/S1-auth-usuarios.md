...
- **Bug encontrado em S6/S7, corrigido aqui (2026-09-21), issue [#13](https://github.com/NicolasNagel/vertere-project/issues/13)**: quando a spec de Pacientes (S4) de fato exercitou a story 12 via `exigir_acao(Acao.PACIENTE_VER)` (`auth/deps.py`, T7), o gate HTTP chamava `authorize(papel, acao)` **sem** `clinica_recurso` — para qualquer ação em `_ACOES_COM_ESCOPO_DE_CLINICA`, isso bloqueava com 403 **todo** usuário `clinica`, mesmo tentando ver dado da própria clínica (o oposto do que a story pede). Não pego pelos testes de S1 porque nenhum teste HTTP com papel `clinica` existia ainda. Corrigido: `exigir_acao()` agora passa a própria clínica do usuário como `clinica_recurso`, o que libera o gate para qualquer `clinica` com a ação concedida — seguro para um endpoint de **listagem** (desde que o service filtre pela clínica do usuário, como `listar_pacientes`/`buscar_pacientes`/`listar_atendimentos`/`listar_laudos` já fazem), mas **não seguro** para um endpoint de recurso único, que deve continuar chamando `authorize()` diretamente após carregar o recurso (ver `laudos.service.ver_laudo`). Ver docstring atualizada de `exigir_acao` em `auth/deps.py`.
