...
export class UsuariosService {
  ...
  async list(page = 1, limit = 20) {
    ...
    const { rows } = await this.pool.query(
      `SELECT id, nome, email, papel, status, crmv, created_at
       FROM usuarios
  ...
  async create(input: CreateUsuarioInput) {
    ...
    const { rows } = await this.pool.query(
      `INSERT INTO usuarios (nome, email, senha_hash, papel, crmv)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, nome, email, papel, status, crmv, created_at`,
      [input.nome, input.email.toLowerCase().trim(), senhaHash, input.papel, input.crmv ?? null],
    );
    return rows[0];
  }
  ...
  async update(id: string, input: UpdateUsuarioInput) {
    ...
    if ('crmv' in input) { fields.push(`crmv = $${i++}`); values.push(input.crmv ?? null); }
