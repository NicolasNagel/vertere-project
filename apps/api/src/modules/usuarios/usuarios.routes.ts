  ...
  app.patch('/usuarios/:id/status', { preHandler: superadminOnly }, async (req, reply) => {
    ...
    if (parsed.data.status === 'inativo') {
      const { rows } = await pool.query(
        `SELECT COUNT(*)::int AS total FROM usuarios WHERE papel = 'superadmin' AND status = 'ativo'`,
      );
      if (rows[0].total <= 1) {
        return reply.status(422).send({ error: 'Não é possível inativar o único superadmin ativo' });
      }
    }
