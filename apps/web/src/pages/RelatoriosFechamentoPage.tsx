...
function exportToExcel(data: FechamentoResponse, clinicaNome: string, dataInicio: string, dataFim: string) {
  ...
  XLSX.writeFile(wb, `fechamento_${clinicaNome.replace(/\s+/g, '_')}_${dataInicio}_${dataFim}.xlsx`);
}

export default function RelatoriosFechamentoPage() {
  ...
  const { data: fechamento, isLoading: loadingFechamento, error: fechamentoError } = useQuery<FechamentoResponse>({
    ...
    queryFn: () => api.get('/relatorios/fechamento', { params: fechamentoSubmitted! }).then((r) => r.data),
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="text-sm text-slate-500 mt-1">Consolidado por empresa e período — exportável para Excel</p>
      </div>
      ...
      {fechamentoSubmitted && (
        <section className="card overflow-hidden mb-8">
          <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-slate-800">{clinicaSelecionada?.nome ?? '—'}</h3>
          ...
          {loadingFechamento ? (
            ...
            <div className="px-5 py-10 text-center text-sm text-red-500">Erro ao carregar os dados.</div>
          ) : !fechamento || fechamento.rows.length === 0 ? (
            <div className="px-5 py-10 text-center text-sm text-slate-400">Nenhum atendimento encontrado no período.</div>
          ) : (
            <>
                      ...
                      <tr key={idx} className="tr">
                        ...
                        <td className="td whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                            r.tipo === 'Plantão' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600'
                          }`}>
                            {r.tipo}
                          </span>
                        </td>
              ...
              <div className="border-t border-slate-200 px-5 py-4 flex flex-wrap justify-end gap-8 bg-slate-50">
                <div className="text-right">
                  ...
                  <p className="text-base font-semibold text-slate-700 tabular-nums">{fmt(fechamento.totais.valor_bruto)}</p>
                </div>
                {fechamento.totais.descontos > 0 && (
                  <div className="text-right">
                    <p className="text-xs text-slate-500 uppercase tracking-wide">Descontos</p>
                    <p className="text-base font-semibold text-red-600 tabular-nums">- {fmt(fechamento.totais.descontos)}</p>
                  </div>
                )}
                <div className="text-right">
                  <p className="text-xs text-slate-500 uppercase tracking-wide">Adicionais</p>
                  <p className="text-base font-semibold text-emerald-700 tabular-nums">{fmt(fechamento.totais.adicionais)}</p>
                </div>
                <div className="text-right border-l border-slate-300 pl-8">
                  ...
                  <p className="text-xl font-bold text-brand-700 tabular-nums">{fmt(fechamento.totais.valor_total)}</p>
                </div>
              </div>
            </>
          )}
