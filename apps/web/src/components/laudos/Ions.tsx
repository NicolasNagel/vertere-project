...
/* ── Referências (confirmadas dos arquivos .docx) ─────────────────────────── */
...
const REFS: Record<EspecieKey, Record<FaixaIdade, IonsRefs>> = {
  cao: {
    filhote: {
...
const PARAMS: { key: string; label: string }[] = [
  { key: 'calcio_ionico', label: 'Cálcio Iônico' },
  { key: 'cloro',         label: 'Cloreto'        },
  { key: 'potassio',      label: 'Potássio'       },
  { key: 'sodio',         label: 'Sódio'          },
  { key: 'fosforo',       label: 'Fósforo'        },
];

export function Ions({ dados, atendimento, signingUser, readOnly, onChange, especie, faixaIdade }: LaudoComponentProps & { especie: EspecieKey; faixaIdade: FaixaIdade }) {
  const d = dados as Record<string, number | string | undefined>;
  const refs = REFS[especie][faixaIdade];
  ...
  const vetReq = atendimento.veterinario_nome
    ...
    : 'Não informado';
  const titulo = `Íons — ${ESPECIE_LABEL[especie]} ${FAIXA_LABEL[faixaIdade]}`;
  ...
  return (
    <div style={{ fontFamily: "'Amasis MT Pro', Georgia, 'Times New Roman', serif", backgroundColor: '#fff', color: TEXT, backgroundImage: 'url(/fundo_laudo.jpg)', backgroundRepeat: 'no-repeat', backgroundPosition: 'center center', backgroundSize: '80%', backgroundBlendMode: 'multiply' }}>
      ...
      <div style={{ padding: '4px 20px 10px', fontSize: 12, color: MUTED, borderBottom: `1px solid ${BORDER}` }}>
        Metodologia: {refs.metodo_cal} para CaI, Cl, K, Na · UV (Molibdato) para Fósforo · Equipamento: EXC-200 Zybio
      </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {PARAMS.map(({ key, label }) => {
            const ref = refs[key as keyof typeof refs] as IonsRef;
            ...
            return (
              <tr key={key}>
                ...
                <td style={valCell}>
                  ...
                  {' '}<span style={{ fontSize: 11, color: MUTED }}>{ref.unit}</span>
                </td>
                <td style={refCell}>{ref.ref}</td>
              </tr>
            );
          })}
        </tbody>
